...
def query_rag_system(query, filters=None, retrieval_options=None, generation_options=None):
...
def start_evaluation(test_dataset, metrics=None):
    ...
    try:
        ...
        response = requests.post(
            f"{st.session_state.api_url}/evaluate",
            json=request_data
        )
...
# Sidebar
st.sidebar.markdown("<div class='main-header'>Enterprise RAG</div>", unsafe_allow_html=True)

# Check API health
api_status, api_details = check_api_health()
if api_status:
    st.sidebar.markdown(
        "<div class='success-box'>✅ API Connected</div>",
        unsafe_allow_html=True
    )
else:
    st.sidebar.markdown(
        "<div class='error-box'>❌ API Unreachable</div>",
        unsafe_allow_html=True
    )
...
# Home page
if page == "home":
    st.markdown("<div class='main-header'>Enterprise-Ready RAG System</div>", unsafe_allow_html=True)
...
# Document Management page
    ...
    with doc_tab1:
        ...
        doc_type = st.selectbox(
            "Document Type",
            ["Auto-detect", "PDF", "DOCX", "Text", "HTML"],
            index=0
        )
        ...
        if uploaded_file is not None:
            if st.button("Upload Document"):
                with st.spinner("Uploading document..."):
                    ...
                    if success:
                        st.success(f"Document uploaded successfully! Document IDs: {result.get('document_ids')}")
                        ...
                        for doc_id in result.get("document_ids", []):
                            st.session_state.documents.append({
                                ...
                                "type": doc_type if doc_type != "Auto-detect" else "Unknown"
                            })
                    else:
                        st.error(f"Error uploading document: {result.get('detail', 'Unknown error')}")
    
    with doc_tab2:
        st.markdown("<div class='sub-header'>Add Text Document</div>", unsafe_allow_html=True)
        ...
        if st.button("Add Document") and text_content:
            with st.spinner("Adding document..."):
                ...
                if success:
                    st.success(f"Document added successfully! Document ID: {result.get('id')}")
                    ...
                    st.error(f"Error adding document: {result.get('detail', 'Unknown error')}")
    ...
    if st.button("Submit Query") and query:
        with st.spinner("Processing query..."):
            ...
            if success:
                ...
                st.error(f"Error processing query: {result.get('detail', 'Unknown error')}")
...
# Evaluation page
    ...
    with eval_tab1:
        st.markdown("<div class='sub-header'>Run Evaluation</div>", unsafe_allow_html=True)
        ...
        if uploaded_file is None:
            ...
            try:
                ...
                if uploaded_file.name.endswith(".jsonl"):
                    # JSONL format
                    test_dataset = [json.loads(line) for line in content.splitlines() if line.strip()]
                else:
                    # JSON format
        ...
        if st.button("Start Evaluation") and test_dataset and metrics:
            with st.spinner("Starting evaluation..."):
                ...
                if success:
                    ...
                    st.error(f"Error starting evaluation: {result.get('detail', 'Unknown error')}")
    
    with eval_tab2:
        st.markdown("<div class='sub-header'>Evaluation Results</div>", unsafe_allow_html=True)
        
        if not st.session_state.evaluation_tasks:
            st.info("No evaluation tasks found. Run an evaluation using the 'Run Evaluation' tab.")
        else:
            # Task selection
            task_options = {f"{task_id} ({data['status']})": task_id for task_id, data in st.session_state.evaluation_tasks.items()}
            ...
            if st.button("Refresh Status"):
                with st.spinner("Refreshing..."):
                    ...
                    if success:
                        ...
                        st.session_state.evaluation_tasks[task_id]["status"] = task_status
                        
                        if task_status == "completed":
                            st.session_state.evaluation_tasks[task_id]["result"] = result.get("result")
                    else:
                        st.error(f"Error refreshing status: {result.get('detail', 'Unknown error')}")
            ...
            if task_data["status"] == "running":
                ...
                if "avg_total_latency_ms" in result.get("overall", {}):
                    ...
                    latency_data = {
                        "Retrieval": result["overall"].get("avg_retrieval_latency_ms", 0),
                        "Augmentation": result["overall"].get("avg_augmentation_latency_ms", 0),
                        "Generation": result["overall"].get("avg_generation_latency_ms", 0),
                ...
                if examples:
                    for i, example in enumerate(examples[:5]):  # Show first 5 examples
                        with st.expander(f"Example {i+1}: {example['query'][:50]}..."):
                            ...
                            if "reference" in example and "answer" in example["reference"]:
                                st.markdown(f"**Reference Answer:** {example['reference']['answer']}")
                            
                            if "response" in example and "answer" in example["response"]:
                                st.markdown(f"**Generated Answer:** {example['response']['answer']}")
...
# Settings page
    ...
    st.markdown("<div class='sub-header'>Default Query Settings</div>", unsafe_allow_html=True)
    ...
    with col1:
        st.session_state.settings["retrieval_type"] = st.selectbox(
            ...
            ["hybrid", "dense", "sparse"],
            index=["hybrid", "dense", "sparse"].index(st.session_state.settings["retrieval_type"])
        )
        
        st.session_state.settings["top_k"] = st.slider(
            "Default Number of Documents",
            1, 20, st.session_state.settings["top_k"]
        )
        
        st.session_state.settings["use_reranker"] = st.checkbox(
            "Use Reranker by Default",
    ...
    with col2:
        st.session_state.settings["model"] = st.selectbox(
            "Default Model",
            ["gpt-3.5-turbo", "gpt-4", "claude-3-sonnet"],
            index=["gpt-3.5-turbo", "gpt-4", "claude-3-sonnet"].index(st.session_state.settings["model"])
        )
        
        st.session_state.settings["temperature"] = st.slider(
            "Default Temperature",
            0.0, 1.0, st.session_state.settings["temperature"], 0.1
        )
        
        st.session_state.settings["citation_format"] = st.selectbox(
            "Default Citation Format",
            ["inline", "footnote"],
            index=["inline", "footnote"].index(st.session_state.settings["citation_format"])
        )
