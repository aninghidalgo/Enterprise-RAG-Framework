    ...
    df = pd.DataFrame(
        {
            ...
            "Score": np.concatenate(
                [
                    [hybrid_scores[i], dense_scores[i], sparse_scores[i]]
                    for i in range(5)
                ]
            ),
        }
    ...
    df = pd.DataFrame(
        {
            ...
            "Metric": np.tile(
                ["Completeness", "Factual Correctness", "Coherence", "Overall Quality"],
                4,
            ),
            "Score": np.concatenate(
                [
                    [completeness[i], correctness[i], coherence[i], overall[i]]
                    for i in range(4)
                ]
            ),
        }
...
def create_retrieval_latency_chart():
    ...
    for i, count in enumerate(doc_counts):
        plt.annotate(
            f"{hybrid_latency[i]} ms",
            (count, hybrid_latency[i]),
            textcoords="offset points",
            xytext=(0, 10),
            ha="center",
        )
        plt.annotate(
            f"{dense_latency[i]} ms",
            (count, dense_latency[i]),
            textcoords="offset points",
            xytext=(0, 10),
            ha="center",
        )
        plt.annotate(
            f"{sparse_latency[i]} ms",
            (count, sparse_latency[i]),
            textcoords="offset points",
            xytext=(0, 10),
            ha="center",
        )
...
def create_hardware_comparison_chart():
    ...
    df = pd.DataFrame(
        {
            ...
            "Value": np.concatenate(
                [
                    [indexing_speed[i], retrieval_latency[i], query_latency[i]]
                    for i in range(3)
                ]
            ),
        }
    ...
    sns.barplot(
        x="Configuration", y="Value", data=df_retrieval, ax=ax2, palette="muted"
    )
