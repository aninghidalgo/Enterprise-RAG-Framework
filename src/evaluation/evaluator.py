...
class EvaluationSuite:
    ...
    def evaluate(
        ...
        for metric in metrics:
            if metric == "latency":
                # Average latencies
                retrieval_latencies = [ex["metrics"].get("retrieval_latency_ms", 0) for ex in results["examples"]]
                augmentation_latencies = [ex["metrics"].get("augmentation_latency_ms", 0) for ex in results["examples"]]
                generation_latencies = [ex["metrics"].get("generation_latency_ms", 0) for ex in results["examples"]]
                total_latencies = [ex["metrics"].get("total_latency_ms", 0) for ex in results["examples"]]
                
                results["overall"]["avg_retrieval_latency_ms"] = sum(retrieval_latencies) / len(retrieval_latencies) if retrieval_latencies else 0
                results["overall"]["avg_augmentation_latency_ms"] = sum(augmentation_latencies) / len(augmentation_latencies) if augmentation_latencies else 0
                results["overall"]["avg_generation_latency_ms"] = sum(generation_latencies) / len(generation_latencies) if generation_latencies else 0
                results["overall"]["avg_total_latency_ms"] = sum(total_latencies) / len(total_latencies) if total_latencies else 0
            else:
                # Average scores
                scores = [ex["metrics"].get(metric, {}).get("score", 0) for ex in results["examples"] if metric in ex["metrics"]]
                
                if scores:
                    results["overall"][f"avg_{metric}_score"] = sum(scores) / len(scores)
    ...
    def _evaluate_example(self, rag_system: Any, example: Dict[str, Any], metrics: List[str]) -> Dict[str, Any]:
        """
