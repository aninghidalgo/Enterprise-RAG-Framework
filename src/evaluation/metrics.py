...
class AnswerMetrics:
    ...
    def semantic_similarity(
        ...
        try:
            ...
            similarity = np.dot(ref_embedding, gen_embedding) / (np.linalg.norm(ref_embedding) * np.linalg.norm(gen_embedding))
...
class HallucinationMetrics:
    ...
    def entailment_score(
        ...
        try:
            ...
            logger.warning("sentence-transformers not available, skipping entailment score")
            return 0.0
        except Exception as e:
    ...
    def fact_verification(
        ...
        try:
            ...
            score_match = re.search(r'hallucination score.*?(\d+(?:\.\d+)?)', response_text, re.IGNORECASE)
...
class CitationMetrics:
    ...
    def citation_verification(
        ...
        try:
            ...
            score_match = re.search(r'accuracy score.*?(\d+(?:\.\d+)?)', response_text, re.IGNORECASE)
            score = float(score_match.group(1)) if score_match else 5.0
