...
def evaluate_example(
    ...
    retrieved_docs = [doc.get("id", "") for doc in response.get("metadata", {}).get("retrieved_docs", [])]
    ...
    for metric in metrics:
        if metric == "retrieval_precision":
            ...
            result["metrics"][metric] = evaluate_citation_accuracy(
                query, reference_context, generated_answer, sources, judge_client, judge_prompts
            )
        elif metric == "latency":
            result["metrics"]["retrieval_latency_ms"] = latency.get("retrieval_ms", 0)
            result["metrics"]["augmentation_latency_ms"] = latency.get("augmentation_ms", 0)
            result["metrics"]["generation_latency_ms"] = latency.get("generation_ms", 0)
            result["metrics"]["total_latency_ms"] = latency.get("total_ms", int(total_time * 1000))
...
def evaluate_answer_relevance(
    ...
    if judge_client and judge_prompts and "answer_relevance" in judge_prompts:
        prompt = judge_prompts["answer_relevance"].format(
            query=query,
            answer=answer
        )
...
def evaluate_factual_correctness(
    ...
    if judge_client and judge_prompts and "factual_correctness" in judge_prompts:
        ...
        try:
            ...
            logger.error("Error in LLM-based factual correctness evaluation: %s", str(e))
...
def evaluate_hallucinations(
    ...
    if judge_client and judge_prompts and "hallucination_detection" in judge_prompts:
        ...
        try:
            ...
            # Extract score
            score_match = re.search(r'(\d+(?:\.\d+)?)', response_text)
            hallucination_score = float(score_match.group(1)) / 10.0 if score_match else 0.5
...
def evaluate_citation_accuracy(
    ...
    citation_precision = CitationMetrics.citation_precision(relevant_sources, source_ids)
    ...
    if judge_client and judge_prompts and "citation_accuracy" in judge_prompts:
        ...
        sources_text = "\n".join([
            f"{s.get('id', 'unknown')}: {s.get('title', 'Untitled')}"
            for s in sources
        ])
