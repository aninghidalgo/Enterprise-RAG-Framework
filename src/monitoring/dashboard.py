...
try:
    ...
    logger.warning(
        "Streamlit or Plotly not installed. Run 'pip install streamlit plotly' to enable the dashboard."
    )
...
class MetricsDashboard:
        ...
        for metric_name, metric_data in latency_metrics.items():
            if "avg" in metric_data:
                ...
                data.append(
                    {
                        "timestamp": timestamp,
                        "metric": metric_name.replace("_seconds", "").replace(
                            "_latency", ""
                        ),
                        "value_ms": value_ms,
    ...
    def get_retrieval_data(self) -> pd.DataFrame:
        ...
        for metric_name, value in retrieval_gauges.items():
            data.append({"timestamp": timestamp, "metric": metric_name, "value": value})
    ...
    def get_query_history(self) -> pd.DataFrame:
        ...
        for query_info in recent_queries:
            entry = {
                ...
                "total_time_ms": query_info.get("metrics", {}).get("total_time", 0)
                * 1000,
                "retrieval_time_ms": query_info.get("metrics", {}).get(
                    "retrieval_time", 0
                )
                * 1000,
                "generation_time_ms": query_info.get("metrics", {}).get(
                    "generation_time", 0
                )
                * 1000,
                "num_docs_retrieved": query_info.get("metrics", {}).get(
    ...
    def render_streamlit_dashboard(self) -> None:
        ...
        if not STREAMLIT_AVAILABLE:
            logger.error(
                "Streamlit is not available. Install with 'pip install streamlit plotly'"
            )
            return
        ...
        # Configure page
        st.set_page_config(
            page_title="RAG System Metrics Dashboard", page_icon="📊", layout="wide"
        )
                ...
                if st.button("Load Selected File"):
                    ...
                    for file_data in metrics_files:
                        if file_data["file"] == selected_file:
                            ...
                            st.write(
                                f"Timestamp: {file_data.get('timestamp', 'Unknown')}"
                            )
            else:
        ...
        with col1:
            ...
            # Query count
            with metric_cols[0]:
                query_count = current_metrics.get("counters", {}).get(
                    "queries_total", 0
                )
            ...
            # Documents processed
            with metric_cols[1]:
                docs_processed = current_metrics.get("counters", {}).get(
            ...
            # Documents indexed
            with metric_cols[2]:
                docs_indexed = current_metrics.get("counters", {}).get(
            ...
            # Cache hits
            with cache_cols[0]:
                cache_hits = current_metrics.get("counters", {}).get(
                    "cache_hits_total", 0
                )
            ...
            # Cache misses
            with cache_cols[1]:
                cache_misses = current_metrics.get("counters", {}).get(
        ...
        if not perf_df.empty and "timestamp" in perf_df.columns:
            ...
            latency_stats = (
                perf_df.groupby("metric")["value_ms"]
            ...
            latency_stats = latency_stats.rename(
                columns={
                    "mean": "Mean (ms)",
                    "min": "Min (ms)",
                    "max": "Max (ms)",
        ...
        if not query_df.empty and "timestamp" in query_df.columns:
            ...
            if len(query_df) > 1:
                ...
                query_count_by_hour = (
                    query_df.groupby("hour").size().reset_index(name="count")
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--port", type=int, default=8501, help="Port to run the Streamlit dashboard on"
    )
    parser.add_argument(
        "--enable-prometheus",
        action="store_true",
...
def main():
    ...
    metrics_registry = get_metrics_registry(
        enable_prometheus=args.enable_prometheus, metrics_dir=args.metrics_dir
    )

    # Create dashboard instance
    dashboard = MetricsDashboard(
        metrics_registry=metrics_registry, metrics_dir=args.metrics_dir
    )
