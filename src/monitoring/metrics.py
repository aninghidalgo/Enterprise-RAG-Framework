...
class MetricsRegistry:
    ...
    def __init__(
        self, enable_prometheus: bool = False, metrics_dir: str = "data/metrics"
    ):
        """
    ...
    def _initialize_default_metrics(self):
        ...
        self.create_counter(
            "document_chunks_created_total", "Total number of document chunks created"
        )
        self.create_counter(
        ...
        self.create_counter(
            "retrieval_operations_total", "Total number of retrieval operations"
        )
        ...
        # System metrics
        self.create_gauge(
            "vector_store_size_bytes", "Size of the vector store in bytes"
        )
    ...
    def record_query(
        self, query: str, response: Dict[str, Any], metrics: Dict[str, Any]
    ) -> None:
        """
        ...
        with self.lock:
            ...
            if "retrieval_time" in metrics:
                self.observe_histogram(
                    "retrieval_latency_seconds", metrics["retrieval_time"]
                )

            if "augmentation_time" in metrics:
                self.observe_histogram(
                    "augmentation_latency_seconds", metrics["augmentation_time"]
                )

            if "generation_time" in metrics:
                self.observe_histogram(
                    "generation_latency_seconds", metrics["generation_time"]
                )

            if "total_time" in metrics:
                self.observe_histogram(
                    "total_query_latency_seconds", metrics["total_time"]
                )

            # Record documents retrieved
            if "num_docs_retrieved" in metrics:
                self.observe_histogram(
                    "documents_retrieved_per_query", metrics["num_docs_retrieved"]
                )

            # Store recent query info
            timestamp = datetime.now().isoformat()
            query_info = {"timestamp": timestamp, "query": query, "metrics": metrics}
...
class QueryMonitor:
    ...
    def end_query(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Collect metrics
        metrics = {"total_time": total_time, "phases": {}}

        for phase, data in self.phase_times.items():
            metrics["phases"][phase] = {"time": data.get("duration", 0)}
            if "metadata" in data:
...
class SystemMonitor:
    ...
    def __init__(
        self, metrics_registry: MetricsRegistry, collection_interval: int = 60
    ):
        """
    ...
    def _initialize_system_metrics(self):
        ...
        self.metrics_registry.create_gauge(
        ...
        self.metrics_registry.create_gauge(
            "process_threads", "Number of threads in the process"
        )

        # System load
        self.metrics_registry.create_gauge(
            "system_load_1min", "System load average (1 min)"
        )
        self.metrics_registry.create_gauge(
            "system_load_5min", "System load average (5 min)"
        )
        self.metrics_registry.create_gauge(
            "system_load_15min", "System load average (15 min)"
        )
    ...
    def collect_system_metrics(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "psutil not installed. System metrics will not be collected."
            )
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"System monitor started with collection interval {self.collection_interval}s"
        )
...
def get_metrics_registry(
    enable_prometheus: bool = False, metrics_dir: str = "data/metrics"
) -> MetricsRegistry:
    """
    ...
    if _metrics_registry is None:
        _metrics_registry = MetricsRegistry(
            enable_prometheus=enable_prometheus, metrics_dir=metrics_dir
        )
