...
class ResponseGenerator:
    ...
    def _format_prompt(
        self, template: Dict[str, str], query: str, context: str
    ) -> Dict[str, str]:
        """
        ...
        # Format user prompt
        user_prompt = template["user"].format(query=query, context=context)
    ...
    def _extract_sources(
        ...
        for citation in citations:
            ...
            source_info = {"id": citation_id, "confidence": 0.0}
            ...
            # Check for numeric citation
                ...
                or f"[Source {citation_id}]" in response_text
            ):
                cited = True
                source_info["confidence"] = 1.0
    ...
    def _calculate_confidence(
        ...
        avg_source_confidence = sum(s.get("confidence", 0.0) for s in sources) / len(
            sources
        )
