...
def parse_args():
    ...
    # Retrieval options
    parser.add_argument(
        "--retrieval-type", choices=["hybrid", "dense", "sparse"], default="hybrid",
...
def format_query_response(response: Dict[str, Any], show_sources: bool = False, show_metadata: bool = False) -> str:
    """
    ...
    if show_sources and "sources" in response and response["sources"]:
        ...
        for i, source in enumerate(response["sources"]):
            output.append(f"{Fore.YELLOW}[{i+1}]{Style.RESET_ALL} {source.get('id', 'Unknown')}")
            ...
            if "metadata" in source:
                output.append(f"    Metadata: {json.dumps(source['metadata'], indent=2)}")
...
def interactive_mode(
    ...
    print(f"Type {Fore.YELLOW}'exit'{Style.RESET_ALL} or {Fore.YELLOW}'quit'{Style.RESET_ALL} to end the session")
    ...
    while True:
        try:
            ...
            if query.lower() == "help":
                ...
                print(f"  {Fore.YELLOW}exit, quit{Style.RESET_ALL} - Exit interactive mode")
                print(f"  {Fore.YELLOW}help{Style.RESET_ALL} - Show this help message")
                print(f"  {Fore.YELLOW}sources on/off{Style.RESET_ALL} - Toggle display of source documents")
                print(f"  {Fore.YELLOW}metadata on/off{Style.RESET_ALL} - Toggle display of response metadata")
                print(f"  {Fore.YELLOW}filter add key=value{Style.RESET_ALL} - Add a metadata filter")
                print(f"  {Fore.YELLOW}filter clear{Style.RESET_ALL} - Clear all filters")
                print(f"  {Fore.YELLOW}filter list{Style.RESET_ALL} - List current filters")
                print(f"  {Fore.YELLOW}topk N{Style.RESET_ALL} - Set number of documents to retrieve")
                print(f"  {Fore.YELLOW}temp N.N{Style.RESET_ALL} - Set temperature for generation")
                print("")
                continue
