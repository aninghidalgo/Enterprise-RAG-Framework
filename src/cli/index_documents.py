...
def get_file_paths(
    input_path: str, file_types: List[str], recursive: bool = False
) -> List[str]:
    """
...
def process_files(
    ...
    for i in range(0, total_files, batch_size):
        ...
        try:
            ...
            logger.info(
                f"Processed batch {i//batch_size + 1}/{(total_files-1)//batch_size + 1}: {len(batch_docs)} documents"
            )
    ...
    logger.info(
        f"Indexing complete: processed {total_files} documents in {elapsed_time:.2f} seconds"
    )
    logger.info(
        f"Average processing time: {elapsed_time/total_files:.2f} seconds per document"
    )
    ...
    try:
        if args.config:
            ...
            rag_system = RAGSystem(
                ...
                vector_store_config={"index_path": args.index_path},
            )
