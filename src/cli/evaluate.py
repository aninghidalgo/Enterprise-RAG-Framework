...
def parse_args():
    ...
    parser.add_argument(
        ...
        "-m",
        type=str,
        default="retrieval_precision,answer_relevance,factual_correctness",
...
def format_evaluation_results(
    results: Dict[str, Any], format_type: str = "text", detailed: bool = False
) -> str:
    """
    ...
    if format_type == "json":
        ...
        output.append(
            f"  Dataset: {results['metadata'].get('dataset_name', 'Unknown')}"
        )
        output.append(
            f"  Number of examples: {results['metadata'].get('num_examples', 0)}"
        )
        output.append(
            f"  Evaluation time: {results['metadata'].get('evaluation_time_seconds', 0):.2f} seconds"
        )
        ...
        for key, value in results["overall"].items():
            if isinstance(value, (int, float)):
                if key.startswith("avg_retrieval_"):
                    ...
                    key.startswith("avg_answer_")
                    or key.startswith("avg_factual_")
                    or key.startswith("avg_hallucination_")
                ):
...
def run_evaluation(
    ...
    if verbose:
        logger.info(
            f"Running evaluation on {len(test_dataset)} examples with metrics: {', '.join(metrics)}"
        )

    results = evaluator.evaluate(
        rag_system=rag_system, test_dataset=test_dataset, metrics=metrics
    )
...
def main():
    ...
    try:
        ...
        formatted_results = format_evaluation_results(
            results, format_type=args.format, detailed=args.detailed
        )
