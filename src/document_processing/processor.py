...
class DocumentProcessor:
    ...
    def process_documents(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in documents:
            try:
                ...
                logger.debug("Processed document into %d chunks: %s", 
                           len(processed_chunks), 
                           doc.get("metadata", {}).get("filename", "unnamed"))
            except Exception as e:
                logger.error("Error processing document %s: %s", 
                           doc.get("metadata", {}).get("filename", "unnamed"), str(e))
                continue
        
        logger.info("Processed %d documents into %d chunks", len(documents), len(processed_docs))
    ...
    def process_document(self, document: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if doc_type in self.processors:
            ...
            logger.warning("No processor found for document type %s, using text processor", doc_type)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if "id" not in chunk:
                ...
                chunk_id = hashlib.md5(f"{chunk_text}_{i}_{time.time()}".encode()).hexdigest()
                chunk["id"] = chunk_id
    ...
    def load_directory(self, directory_path: str) -> List[Dict[str, Any]]:
        ...
        logger.info("Loaded %d documents from directory %s", len(documents), directory_path)
        return documents
    ...
    def load_document(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        if file_extension == ".pdf" and "pdf" in self.processors:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                content = f.read()
        elif file_extension in [".jpg", ".jpeg", ".png", ".gif", ".bmp"] and "image" in self.processors:
            doc_type = "image"
            content = self.processors["image"].extract_text(file_path)
        elif file_extension in [".mp3", ".wav", ".ogg", ".m4a"] and "audio" in self.processors:
            doc_type = "audio"
