...
class ContextAugmenter:
    ...
    def _initialize_compression_model(self) -> Any:
        ...
        try:
            ...
            logger.warning("Failed to initialize compression model. Please install transformers library.")
            return None
    ...
    def augment(
        ...
        if strategy == "simple":
            ...
            context_text, citations = self._relevance_weighted_context(documents, config)
        elif strategy == "semantic_clustering":
            context_text, citations = self._semantic_clustering_context(documents, query, config)
        else:
            logger.warning("Unknown context strategy: %s, using simple context", strategy)
            context_text, citations = self._simple_context(documents, config)
    ...
    def _semantic_clustering_context(
        ...
        try:
            ...
            sorted_clusters = sorted(clustered_docs.keys(), key=get_cluster_score, reverse=True)
            ...
            for cluster_id in sorted_clusters:
                ...
                cluster_budget = int(remaining_budget * (score / sum(get_cluster_score(c) for c in sorted_clusters)))
            ...
            for cluster_id in sorted_clusters:
                ...
                sorted_docs = sorted(docs, key=lambda x: x.get("reranker_score", x.get("score", 0)), reverse=True)
                ...
                for doc in sorted_docs:
                    ...
                    if cluster_length + text_length > budget:
                        ...
                        if truncated_length > 50:  # Only add if we can include meaningful content
            ...
            logger.warning("scikit-learn not available, falling back to relevance weighted context")
            return self._relevance_weighted_context(documents, config)
        except Exception as e:
