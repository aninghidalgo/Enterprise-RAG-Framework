...
class FAISSVectorStore:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized FAISS vector store with dimension {self.embedding_dimension}"
        )
        ...
        try:
            ...
            if index_type == "flat":
                ...
                if self.config.get("random_training", False):
                    ...
                    train_vectors = np.random.random((train_size, dimension)).astype(
                        np.float32
                    )
            ...
            logger.error(
                "Failed to initialize FAISS. Please install with: pip install faiss-cpu or faiss-gpu"
            )
            raise
        except Exception as e:
    ...
    def save_index(self, index_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved FAISS index with {len(self.docstore)} documents to {index_path}"
            )
    ...
    def delete_documents(self, document_ids: List[str]) -> None:
        ...
        if not self.config.get("support_remove", False):
            logger.warning(
                "Document deletion not supported in current FAISS index, rebuilding index"
            )
        ...
        try:
            ...
            if indices_to_remove and hasattr(self.index, "remove_ids"):
                ...
                logger.info(
                    f"Removed {len(indices_to_remove)} documents from FAISS index"
                )
            else:
    ...
    def _rebuild_index_without(self, document_ids_to_remove: List[str]) -> None:
        ...
        try:
            ...
            logger.info(
                f"Rebuilt FAISS index with {len(docs_to_keep)} documents, removed {len(document_ids_to_remove)}"
            )
