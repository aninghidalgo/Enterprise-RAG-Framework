...
class RetrievalEngine:
    ...
    def __init__(
        ...
        logger.info(
            "Initializing retrieval engine with config: %s",
            {
    ...
    def _initialize_reranker(self) -> Any:
        ...
        """
        reranker_model = self.retrieval_config.get(
    ...
    def index_documents(self, documents: List[Dict[str, Any]]) -> None:
        ...
        logger.info(
            "Indexed %d documents in %.2f seconds", len(documents), elapsed_time
        )
    ...
    def retrieve(
        ...
        retrieval_type = options.get(
            "retrieval_type", self.retrieval_config.get("type", "hybrid")
        )
        use_reranker = options.get(
            "rerank", self.retrieval_config.get("use_reranker", True)
        )

        # Select retrieval strategy
        if retrieval_type == "dense":
            retrieved_docs = self._dense_retrieval(
            ...
            retrieved_docs = self._hybrid_retrieval(
                query, top_k, similarity_threshold, filters
            )
        else:
            logger.warning(
                "Unknown retrieval type: %s, using hybrid retrieval", retrieval_type
            )
            retrieved_docs = self._hybrid_retrieval(
                query, top_k, similarity_threshold, filters
            )
        ...
        if use_reranker and self.reranker is not None and len(retrieved_docs) > 1:
            ...
            retrieved_docs = self._rerank_documents(
                query, retrieved_docs, reranker_threshold
            )
        ...
        if doc_count > 0:
            ...
            min_score = (
                retrieved_docs[-1].get("score", 0) if doc_count > 1 else max_score
            )
    ...
    def _dense_retrieval(
        ...
        filtered_results = [
            doc for doc in results if doc.get("score", 0) >= similarity_threshold
        ]
    ...
    def _sparse_retrieval(
        ...
        if self.sparse_retriever is None:
            logger.warning(
                "Sparse retriever not initialized, falling back to dense retrieval"
            )
    ...
    def _rerank_documents(
        self, query: str, documents: List[Dict[str, Any]], score_threshold: float = 0.4
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            reranked_docs = sorted(
                filtered_docs, key=lambda x: x.get("reranker_score", 0), reverse=True
            )
